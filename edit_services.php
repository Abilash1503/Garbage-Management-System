...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    $zip=$_POST["zip"];
        ...
        $service_date = date("Y-m-d", time() + 86400);
            $sql = "INSERT INTO `application_request`(`garbage_type`,`weight`, `login_username`, `permission`, `fname`, `service_date`,`route_name`,`address`,`state`,`city`,`zip`) values('$garbage_type','$weight' ,'$login_username', '2', '$fname','$service_date','$route_name','$address','$state','$city','$zip')";
            
            $result = mysqli_query($conn, $sql);
            if($result){
				$select_driver= "SELECT * FROM `driver` WHERE vehicle_unique_id='$vehicle_unique_id'";
				$result4 = mysqli_query($conn, $select_driver);
				while($row5=mysqli_fetch_assoc($result4)){
       
        
					$driver_unique_id=$row5['driver_unique_id'];
					$vehicle_unique_id=$row5['vehicle_unique_id'];
				}
			

				$sql_update = "UPDATE `schedule` SET `driver_unique_id`='$driver_unique_id'  WHERE `vehicle_unique_id`='$vehicle_unique_id'";
            
				$result3 = mysqli_query($conn, $sql_update);

				$select_garbage= "SELECT * FROM `vehicle` WHERE vehicle_unique_id='$vehicle_unique_id'";
				$result5 = mysqli_query($conn, $select_garbage);
				while($row6=mysqli_fetch_assoc($result5)){

					$garbage_type=$row6['garbage_type'];
					$color=$row6['color'];
					$vehicle_unique_id=$row6['vehicle_unique_id'];
				}
				$sql_update_garbage = "UPDATE `schedule` SET `garbage_type`='$garbage_type',`color`='$color'  WHERE `vehicle_unique_id`='$vehicle_unique_id'";
            
				$result7 = mysqli_query($conn, $sql_update_garbage);
						...
						<div class="card-body">
							...
							<div class="row mb-3">
                                ...
                                <select name="garbage_type" class="form-control" id="garbage_type" required="required">
                                	<?php 
									$sql_option1="select distinct garbage_type from garbage_category where service_available='available' and garbage_id='$garbage_id' ;";
                            ...
                            <div class="row mb-3">
								<div class="col-sm-3">
									<h6 class="mb-0">Route and nearest stop</h6>
								</div>
							 	<div class="col-sm-9 text-secondary">
